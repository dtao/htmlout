require 'should'
StringWriter = require('writer.js').StringWriter
...
getOutput = (html, css) ->
  ...
  fn = htmlout
  if css
    fn = htmlout.withCSS(css)
  fn(html, { writer: writer })
  writer.toString()

describe 'htmlout', ->
  it 'reads style attributes directly', ->
    getOutput('<span style="color: #f00;">foo</span>').should.eql(
      '\x1B[91mfoo\x1B[39m'
    )
  ...
  it 'defaults to treating whitespace as insignificant', ->
    html =
      ...
      '''
    getOutput(html).should.eql('This is a paragraph. It has two spaces after ' +
  ...
  it 'understands stylesheets', ->
    css =
      ...
      .highlight { color: #ff0; }
      '''
    getOutput('<span class="highlight">blah</span>', css).should.eql(
      '\x1B[93mblah\x1B[39m'
    )

  it 'can apply extended colors', ->
    getOutput('<span style="color: #005FFF;">howdy</span>').should.eql(
      '\x1B[38;5;27mhowdy\x1B[39m'
    )

  it 'can apply extended background colors', ->
    getOutput('<span style="background-color: #005FFF;">howdy</span>').should.eql(
      '\x1B[48;5;27mhowdy\x1B[49m'
    )

  it 'can apply the text-transform property', ->
    getOutput('<span style="text-transform: uppercase;">shout</span>')
      .should.eql('SHOUT')

  it 'defaults <strong> and <b> elements to bold', ->
    getOutput('<strong>foo</strong> bar <b>baz</b>').should.eql(
      '\x1B[1mfoo\x1B[22m bar \x1B[1mbaz\x1B[22m'
    )

  it 'defaults <em> and <i> elements to italic', ->
    getOutput('<em>foo</em> bar <i>baz</i>').should.eql(
      '\x1B[3mfoo\x1B[23m bar \x1B[3mbaz\x1B[23m'
    )

  it 'defaults <u> elements to underlined', ->
    getOutput('<u>blah</u>').should.eql(
      '\x1B[4mblah\x1B[24m'
    )

  it 'defaults <strike> and <del> elements to strikethrough', ->
    getOutput('<strike>struck</strike> and <del>deleted</del>').should.eql(
